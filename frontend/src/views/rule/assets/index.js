    ...
    methods: {
                ...
                >
                    ...
                    >
                        <i-button
                            ...
                            disabled={data.type == "rule" ? true : false}
                        >
                            {__("子权限")}
                        </i-button>
                        <i-button
        ...
        edit(root, node, nodeData) {
            ...
            data.value = data.value
                ? data.value.split(",").map(item => parseInt(item))
                : [];
            ...
            setTimeout(() => {
                ...
                this.dataMenuTree = this.setMenuCheckedNodes(
                    this.initMenuTree,
                    data.value
                );
            }, 0);
        ...
        setMenuCheckedNodes(menuTree, value) {
            ...
            menuTree.forEach((el, index) => {
                ...
                if (el.children) {
                    item.children = this.setMenuCheckedNodes(
                        el.children,
                        value
                    );
                } else {
        ...
        order(root, node, nodeData, type) {
            ...
            this.apiPut("menu/order/", nodeData.id, { type: type }).then(
                res => {
                    utils.success(res.message);
        ...
        saveMenu(form) {
            this.apiPost("rule", this.formItem).then(
                res => {
                    const addNode = {
