            ...
            <div style="text-align: center;" :key="index">
                ...
                >
                    <i-button style="width: 70px;margin-left: -5px;padding:10px 0;" type="text">
                        <Icon :v-if="item.icon" :size="20" :color="iconColor" :type="item.icon" :style="!item.permission ? 'color:#c5c8ce;' : ''"></Icon>
                    </i-button>
                    <DropdownMenu style="width: 150px;" slot="list">
                        <template v-for="(child, i) in item.children">
                            <DropdownItem :name="child.name" v-if="!child.children || child.children.length <= 0" :key="i" :disabled="!child.permission"
                            ...
                            >
                                <DropdownItem>
                                    <span :style="!child.permission ? 'color:#c5c8ce;padding-left:10px;' : 'padding-left:10px;'">
                                        {{ itemTitle(child) }}
                                ...
                                <DropdownMenu slot="list">
                                    <template v-for="childsub in child.children" :name="childsub.name">
                                        <DropdownItem style="width: 150px;" :name="childsub.name">
                                            <Icon :v-if="childsub.icon" :type="childsub.icon" v-if="childsub.icon"></Icon>
                                            <span style="padding-left:10px;">
                ...
                <Dropdown transfer v-else placement="right-start" :key="index" @on-click="changeMenu">
                    ...
                    <DropdownMenu style="width: 200px;" slot="list">
                        <DropdownItem :name="item.children[0].name" :key="'d' + index"
                            ><Icon :v-if="item.icon" :type="item.icon" :style="!item.children[0].permission ? 'color:#c5c8ce;' : ''"></Icon
