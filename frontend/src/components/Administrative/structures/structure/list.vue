...
<div>
    ...
    <div class="content-main2">
        ...
        <el-tree class="filter-tree" :data="dataTree" :props="defaultProps" :filter-node-method="filterNode" ref="tree" node-key="id" show-checkbox default-expand-all highlight-current check-strictly="true" :render-content="renderContent">
        </el-tree>
...
export default {
    methods: {
        ...
        enable(node, data, store, event, status) {
            this.stopPropagation(event)
            this.apiPut('/admin/structure/enable/', data.id, {
                status: status
            }).then((res) => {
        ...
        order(node, data, store, event, type) {
            this.stopPropagation(event)
            this.apiPut('/admin/structure/order/', data.id, {
                type: type
            }).then((res) => {
                this.handelResponse(res, (data) => {
                    _g.toastMsg('success', res.message)
