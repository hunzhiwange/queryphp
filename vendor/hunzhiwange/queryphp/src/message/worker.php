...
class worker extends Resque_Worker
{
	...
	{
		$workers = static::queue()->storage()->smembers('workers');
		if(!is_array($workers)) {
			$workers = array();
		}
	...
	{
		return (bool)static::queue()->storage()->sismember('workers', $workerId);
	}
	...
	public function reserve()
	{
		...
		foreach($queues as $queue) {
			...
			//exit();
			$job = call_user_func_array(['Q\message\\'.static::$objQueue->getType().'_job', 'reserve'],[$queue]);
			if($job) {
	...
	{
		static::queue()->storage()->sadd('workers', $this);
		static::queue()->storage()->set('worker:' . (string)$this . ':started', strftime('%a %b %d %H:%M:%S %Z %Y'));
	}
	...
	public function unregisterWorker()
	{
		...
		$id = (string)$this;
		static::queue()->storage()->srem('workers', $id);
		static::queue()->storage()->del('worker:' . $id);
		static::queue()->storage()->del('worker:' . $id . ':started');
	 ...
	 */
	public function workingOn2( $job)
	{
		$job->worker = $this;
		$this->currentJob = $job;
		...
		self::$objQueue->storage()->set('worker:' . $job->worker, $data);
	}
