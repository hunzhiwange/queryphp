...
class status extends Resque_Job_Status
{
	...
	public static function create($id)
	{
		...
		static::queue()->storage()->set('job:' . $id . ':status', json_encode($statusPacket));
	}
	...
	public function isTracking()
	{
		...
		if(!static::queue()->storage()->exists((string)$this)) {
	...
	public function get()
	{
		...
		$statusPacket = json_decode(static::queue()->storage()->get((string)$this), true);
		if(!$statusPacket) {
			return false;
		}
