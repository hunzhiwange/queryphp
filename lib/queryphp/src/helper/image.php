        ...
        if (! empty ( $sBackgroundPath ) && is_file ( $sBackgroundPath )) {
            if ($arrWaterArgs ['type'] == 'img' && ! empty ( $arrWaterArgs ['path'] )) {
                ...
                if (isset ( $arrWaterArgs ['textPath'] ) && strtolower ( $arrWaterArgs ['textPath'] ) == 'framework-font') { // 使用框架内部提供的字体
                    $arrWaterArgs ['textPath'] = Q_PATH . '/resource/font';
                    $bFrameworkFont = true;
                }
                
                $sFontfileTemp = $sFontfile = isset ( $arrWaterArgs ['textFile'] ) && ! empty ( $arrWaterArgs ['textFile'] ) ? $arrWaterArgs ['textFile'] : 'pilsen_plakat.ttf';
                $sFontfile = (! empty ( $arrWaterArgs ['textPath'] ) ? Q::tidyPath ( $arrWaterArgs ['textPath'] ) . ($bFrameworkFont === true ? '/en-us' : '') : 'C:\WINDOWS\Fonts') . '/' . $sFontfile;
                if ($bFrameworkFont === true && ! is_file ( $sFontfile )) { // 这里如果为框架内部字体的话，在 en-us 中找不到，那么到 zh-cn 中去找字体
                    $sFontfile = Q::tidyPath ( $arrWaterArgs ['textPath'] ) . '/zh-cn/' . $sFontfileTemp;
                }
