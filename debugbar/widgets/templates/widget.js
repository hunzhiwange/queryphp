  ...
  var TemplatesWidget = (PhpDebugBar.Widgets.TemplatesWidget = PhpDebugBar.Widget.extend({
    ...
    render: function() {
        ...
        itemRenderer: function(li, tpl) {
          ...
          if (tpl.render_time_str) {
            $('<span title="Render time" />')
              .addClass(csscls('render-time'))
              .text(tpl.render_time_str)
              .appendTo(li)
          }
          if (tpl.memory_str) {
          ...
          if (typeof tpl.param_count != 'undefined') {
            $('<span title="Parameter count" />')
              .addClass(csscls('param-count'))
              .text(tpl.param_count)
              .appendTo(li)
          }
      ...
      this.bindAttr('data', function(data) {
        ...
        $('<span />')
          .text(data.nb_templates + ' ' + sentence)
          .appendTo(this.$status)

        if (data.accumulated_render_time_str) {
          this.$status.append(
            $('<span title="Accumulated render time" />')
              .addClass(csscls('render-time'))
              .text(data.accumulated_render_time_str)
          )
        }
        if (data.memory_usage_str) {
          this.$status.append(
            $('<span title="Memory usage" />')
              .addClass(csscls('memory'))
              .text(data.memory_usage_str)
          )
        }
        if (data.nb_blocks > 0) {
          $('<div />')
            .text(data.nb_blocks + ' blocks were rendered')
            .appendTo(this.$status)
        }
        if (data.nb_macros > 0) {
          $('<div />')
            .text(data.nb_macros + ' macros were rendered')
            .appendTo(this.$status)
        }
