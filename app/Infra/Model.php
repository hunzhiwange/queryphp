...
abstract class Model
{
    ...
    {
        if ('getby' === strtolower(substr($method, 0, 5))) {
            // 根据某个字段获取记录
        ...
        if ('getfieldby' === strtolower(substr($method, 0, 10))) {
            // 根据某个字段获取记录的某个值
        ...
        if (isset($this->_scope[$method])) {// 命名范围的单独调用支持
    ...
    public function select(int|string|array $options = []): array
    {
        ...
        if (isset($options['index'])) { // 对数据集进行索引
            ...
            foreach ($resultSet as $result) {
                $_key = $result[$index[0]];
                if (isset($index[1], $result[$index[1]])) {
    ...
    public function buildSql(): string
    {
        try {
            ...
            $sql = $this->database->buildSelectSqlForSelect($options);

            return '( '.$sql.' )';
        } finally {
            $this->options['fetch_sql'] = false;
        }
    }
     ...
     * 查询数据.
     */
    public function find(array|string|int|float $options = []): array|string
    {
        $where = [];
    ...
    {
        if (null !== $parse && \is_string($where)) {
            if (!\is_array($parse)) {
                $parse = \func_get_args();
                array_shift($parse);
            }
            $parse = array_map([$this->database, 'escapeString'], $parse);
     ...
     * 通过 Select 方式添加记录.
     */
    public function selectAdd(array|string $fields = '', string $table = '', array $options = []): int|string
    {
        // 分析表达式
        $options = $this->_parseOptions($options);
        // 写入数据到数据库
     ...
     * @throws \Exception
     */
    public function save(mixed $data = '', array $options = []): int
    {
        if (empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if (!empty($this->data)) {
     ...
     * SQL 查询.
     */
    public function query(string $sql, bool|array|string $parse = false): array
    {
    ...
    {
        if (null === $length && \is_string($offset) && strpos($offset, ',')) {
            [$offset, $length] = explode(',', $offset);
        }
        $this->options['limit'] = (int) $offset.($length ? ','.(int) $length : '');
     ...
     *
     * @param mixed      $page
     * @param null|mixed $listRows
     */
    public function page($page, $listRows = null): static
    {
        if (null === $listRows && \is_string($page) && strpos($page, ',')) {
            [$page, $listRows] = explode(',', $page);
        }
        $this->options['page'] = [(int) $page, (int) $listRows];
        ...
        if (isset($validateData)) { // 如果设置了数据自动验证则进行数据验证
            ...
            foreach ($validateData as $val) {
                ...
                if (empty($val[5]) || (self::MODEL_BOTH === $val[5] && $type < 3) || $val[5] === $type) {
                    if (str_starts_with($val[2], '{%') && strpos($val[2], '}')) {
                        // 支持提示信息的多语言 使用 {%语言定义} 方式
                        $val[2] = substr($val[2], 2, -1);
                    }

                    $val[3] ??= self::EXISTS_VALIDATE;
                    $val[4] ??= 'regex';
                    // 判断验证条件
                    switch ($val[3]) {
