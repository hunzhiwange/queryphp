...
use App\Product\Entity\BaseBrandModel;
...
final class ModelTest extends TestCase
{
    protected function tearDown(): void
    {
        ...
        // 恢复公司默认值，否则会导致其它地方的测试用例失败
        container()->instance('company_id', 100100);
    }
    ...
    public function testQuerySub57(): void
    {
        $baseBrandModel = BaseBrandModel::make();
        $baseBrandModel
            ->field('brand.brand_name,p.title')
            ->table('base_brand brand,product p')
            ->limit(10)
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  brand.brand_name,p.title FROM base_brand brand,product p LIMIT 10';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub59(): void
    {
        ...
        $baseBrandModel
            ->alias('a')
            ->join('product b ON b.id= a.brand_id')
            ->limit(10)
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  * FROM base_brand a INNER JOIN product b ON b.id= a.brand_id  LIMIT 10';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub100(): void
    {
        $baseBrandModel = BaseBrandModel::make();
        $baseBrandModel
            ->join('product ON product.id = `base_brand`.brand_id')
            ->join('product_brand ON product_brand.id = `base_brand`.brand_id')
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  * FROM `base_brand` INNER JOIN product ON product.id = `base_brand`.brand_id INNER JOIN product_brand ON product_brand.id = `base_brand`.brand_id';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub101(): void
    {
        ...
        $baseBrandModel
            ->join([
                'product ON product.id = `base_brand`.brand_id',
                'product_brand ON product_brand.id = `base_brand`.brand_id',
            ])
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
    ...
    public function testQuerySub102(): void
    {
        $baseBrandModel = BaseBrandModel::make();
        $baseBrandModel
            ->join('LEFT JOIN product ON product.id = `base_brand`.brand_id')
            ->join('LEFT JOIN product_brand ON product_brand.id = `base_brand`.brand_id')
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  * FROM `base_brand` LEFT JOIN product ON product.id = `base_brand`.brand_id LEFT JOIN product_brand ON product_brand.id = `base_brand`.brand_id';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub103(): void
    {
        $baseBrandModel = BaseBrandModel::make();
        $baseBrandModel
            ->join('RIGHT JOIN product ON product.id = `base_brand`.brand_id')
            ->join('RIGHT JOIN product_brand ON product_brand.id = `base_brand`.brand_id')
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  * FROM `base_brand` RIGHT JOIN product ON product.id = `base_brand`.brand_id RIGHT JOIN product_brand ON product_brand.id = `base_brand`.brand_id';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub104(): void
    {
        ...
        $baseBrandModel
            ...
            ->table('base_brand')
            ->union('SELECT title as name FROM product')
            ->union('SELECT name FROM product_brand')
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  `brand_name` as name FROM `base_brand` UNION SELECT title as name FROM product UNION SELECT name FROM product_brand';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub105(): void
    {
        ...
        $baseBrandModel
            ->field('`brand_name` as name')
            ->table('base_brand')
            ->union(['field' => 'title as name', 'table' => 'product'])
            ->union(['field' => 'name', 'table' => 'product_brand'])
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
    ...
    public function testQuerySub106(): void
    {
        ...
        $baseBrandModel
            ->field('`brand_name` as name')
            ->table('base_brand')
            ->union(['SELECT title as name FROM product', 'SELECT name FROM product_brand'])
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
    ...
    public function testQuerySub107(): void
    {
        ...
        $baseBrandModel
            ->field('`brand_name` as name')
            ->table('base_brand')
            ->union('SELECT title as name FROM product', true)
            ->union('SELECT name FROM product_brand', true)
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
        $sql = 'SELECT  `brand_name` as name FROM `base_brand` UNION ALL SELECT title as name FROM product UNION ALL SELECT name FROM product_brand';
        static::assertSame($result, $sql);
    }
    ...
    public function testQuerySub108(): void
    {
        ...
        $baseBrandModel
            ->field('`brand_name` as name')
            ->table('base_brand')
            ->union(['SELECT title as name FROM product', 'SELECT name FROM product_brand'], true)
            ->select()
        ;
        $result = $baseBrandModel->getLastSql();
        $result = trim($baseBrandModel->getLastSql());
    ...
    public function testQuerySub200(): void
    {
        ...
        container()->instance('company_id', 0);
        $baseBrandModel = BaseBrandModel::make();
        $id = $baseBrandModel->transaction(fn () => $baseBrandModel
            ->trans2([
                'first' => 'new1',
                'second' => 'new2',
            ]));
    ...
    public function testQuerySub215(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
        ...
        $result = trim($baseBrandModel->getLastSql());
        $sql = "REPLACE INTO `base_brand` (`brand_name`,`brand_logo`) VALUES ('hello','123'),('world','4')";
        static::assertSame($result, $sql);
    }

    public function testQuerySub216(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
        ...
        $result = trim($baseBrandModel->getLastSql());
        $sql = "REPLACE INTO `base_brand` (`brand_name`,`brand_logo`) VALUES ('hello','123')";
        static::assertSame($result, $sql);
    }

    public function testQuerySub217(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
        ...
        $result = trim($baseBrandModel->getLastSql());
        $sql = "INSERT INTO `base_brand` (`brand_name`,`brand_logo`) VALUES ('hello','123') ON DUPLICATE KEY UPDATE `brand_logo`=VALUES(`brand_logo`)";
        static::assertSame($result, $sql);
    }

    public function testQuerySub218(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
    ...
    public function testQuerySub219(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
        ...
        $baseBrandModel->addAll(
            ...
            [],
            [
                'brand_logo' => ['exp', "concat ( VALUES ( `brand_name` ), 'xx', VALUES ( `brand_logo` ) )"],
            ],
        );
        $result = trim($baseBrandModel->getLastSql());
        $sql = "INSERT INTO `base_brand` (`brand_name`,`brand_logo`) VALUES ('hello','123'),('world','4') ON DUPLICATE KEY UPDATE `brand_logo`=(concat ( VALUES ( `brand_name` ), 'xx', VALUES ( `brand_logo` ) ))";
        static::assertSame($result, $sql);
    }

    public function testQuerySub220(): void
    {
        container()->instance('company_id', 999);
        http_request()->request->set('brand_name', 'hello');
        http_request()->server->set('REQUEST_METHOD', Request::METHOD_POST);
        ...
        $result = trim($baseBrandModel->getLastSql());
        $sql = "INSERT INTO `base_brand` (`brand_name`,`brand_logo`) VALUES ('hello','123'),('world','4') ON DUPLICATE KEY UPDATE `brand_logo`='5'";
        static::assertSame($result, $sql);
    }
}
