...
trait ParseEntityMethod
{
    protected function parseEntityMethod(string $entityClass, string $entityMethod): array
    {
        ...
        if (str_contains($entityClass, ':')) {
            ...
            // @todo 移除目录的设计
            if (str_contains($entityClass, ':')) {
                $entityClass = implode('\\', array_map(static fn ($v) => ucfirst($v), explode(':', $entityClass)));
            }
        }
        if (str_contains($entityClass, '_')) {
            $entityClass = implode('', array_map(static fn ($v) => ucfirst($v), explode('_', $entityClass)));
        }

        if (str_contains($entityMethod, ':')) {
            $entityMethod = implode('\\', array_map(static fn ($v) => ucfirst($v), explode(':', $entityMethod)));
        }
        if (str_contains($entityMethod, '_')) {
            $entityMethod = implode('', array_map(static fn ($v) => ucfirst($v), explode('_', $entityMethod)));
        }
    ...
    private function runEntityMethod(string $sourceEntityClass, ParamsDto $params): array|Response
    {
        ...
        $entityService = \App::proxy()->make($entityServiceClass);
